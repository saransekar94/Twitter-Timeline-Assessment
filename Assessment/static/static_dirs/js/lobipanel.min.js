Math.randomString = function(n) {
    ...
    var LobiPanel = function($el, options) {
        ...
        var $heading, $body, innerId, storagePrefix = "lobipanel_",
            ...
            _generateCustom = function() {
                var options = me.$options.custom,
                    control = $('<a data-func="custom"></a>');
                return control.append('<i class="' + LobiPanel.PRIVATE_OPTIONS.iconClass + " " + options.icon + '"></i>'), options.tooltip && "string" == typeof options.tooltip && (control.append('<span class="control-title">' + options.tooltip + "</span>"), control.attr("data-tooltip", options.tooltip)), _onCustomClick(control), $("<li></li>").append(control)
            },
            _onCustomClick = function(control) {
                control.on("mousedown", function(ev) {
                    //ev.stopPropagation()
                }), control.on("click", function() {
                    //me.togglePin()
                    display = me.$el[0].childNodes[1].childNodes[0].style.display;
                    if(display=="none"){
                        me.$el[0].childNodes[1].childNodes[0].style.display = "block";
                        me.$el[0].childNodes[1].childNodes[1].style.display = "none";
                    }
                    else{
                        me.$el[0].childNodes[1].childNodes[0].style.display = "none";
                        me.$el[0].childNodes[1].childNodes[1].style.display = "block";
                    }
                })
            },
